 ...
 * Ensures [WorkoutSessionDto.toEntity] dayKey matches the system zone's calendar day of session start.
 */
class WorkoutSessionIdentityMappingTest {

    @Test
    fun toEntity_dayKey_matchesLocalStartDate_inSystemZone() {
        val zone = Clock.systemDefaultZone().zone
    ...
    fun toDomain_roundTrip_preservesInstants() {
        ...
        val back = session.toDto().toEntity().toDomain()
