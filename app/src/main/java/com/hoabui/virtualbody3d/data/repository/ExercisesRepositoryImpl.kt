...
import com.hoabui.virtualbody3d.data.mapper.toFeedExercise
...
class ExercisesRepositoryImpl @Inject constructor(
    ...
    override fun getExercises(): Flow<List<FeedExercise>> =
        localDataSource.getExercises().map { list -> list.map { it.toFeedExercise() } }

    override fun getExercisesByDay(day: LocalDate): Flow<List<FeedExercise>> =
        localDataSource.getExercisesByDay(day).map { list -> list.map { it.toFeedExercise() } }
}
